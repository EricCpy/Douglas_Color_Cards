...
* We discussed about how to best specify out hypotheses as a model. For example regarding the question if we should use cardnumber as a factor or as numeric.
* We have concentrated to establish common color scales for similar visualisation and using other ones for differing scales. It seems really hard to have a set of distinguishable colors when each color should be linked to something all over the report.
...
* New and fancy visualisations are not always better. We found violin plots to have some problems and tackeled one of those.
* The heavy package is archived. To perform a outlier resistent regression (with T distributed errors) one could user bayesian regression with *brms*. But we didn't perform this analysis here. The trend spotted with ones eye can get disturbed realy quick by some off values.
* We are not sure if our ordinary bootstrap approach is faulty or if the 3rd order interaction model does really overfit very strongly and the perumtative bootstrapping approch is wrong. We should contact an expert about this instead of hoping that the answer we like more is true or being too defensive with our decision.
* We learned that it is easy to forget how to do some specific things with R / ggplot. Even though I worked on a visualisation package for bayesian models I forgot how to color geom_ridges differently. But once you know that it is possible one finds possible solutions on Stack Overflow fast. Maybe I should have tested an generative AI on this (or the violins) question? I think when I had to redo this I'ld ask a generative AI to help me with some coding tasks.
