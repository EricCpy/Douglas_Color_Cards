...
We can see, that \( \Delta \)E generally goes up the further down we go and that there are three columns of higher \( \Delta \)E values. But those columns are not the high numbered but every second. Thus our hypothesis was not correct. But nevertheless: significant difference was found.
...
In the following graphic one can see additionally that the densities for the color difference median has really thick tails to the negative side if the 3rd order interaction model is part of the comparison. This shows that the difference between the performance of the models can be really huge. One might not only get a over parameterized model that gets a little bit worse suggestions. It is possible to get a model that predicts realy weird colors.
...
One should take the safer option (2nd order interaction model) if the colors should be used in a traffic light and wrong colors could lead to traffic accidents, injuries and death. If its just the color of an add in a video game it does not matter. But even here one would be totally fine with the 2nd order interaction model and it would be computative less costly as well. I challange the reader to come up with a use case where the small advantage of a good 3rd order interaction model could be crucial.