...
1. How is the color special used (regression)? (Simon: R-Code ✔, Text ✔)
...
As we can see, there is a significant evidence, that the mean dE is different at different card positions. Now, let's visualize it. 
...
Exact meaning of the obtained information depends on the printing process involved. We can see, that dE generally goes up the further down we go, and that there are three columns of higher dE values.
...
### Comparative analysis of \( \Delta \)E for skin- and non-skin colors

The Douglas color card is not uniform, but rather it consists of 3 distinct areas:

1. Outer rows and columns (rows 1 and 8, columns 1 and 8), as well as well as cells [2;2], [2;7], [3;5] and [6;5] that contain non-skin colors.
2. Central 4 cells (intersection of rows 4 and 5 with columns 4 and 5) that are grey, and would be removed in the finished product.
3. The rest of the card, which contains actual skin colors.

As part of the evaluation of the printing process, it may be interesting to compare the results of the printing for skin versus non-skin colors, as Douglas printing process specializes on the former.

```{r, echo=FALSE, echo=FALSE, fig.width=8, fig.height=6, cache=!full_refit}
...
As we can see, there is a group of outlier cells that contain skin colors that have wildly higher \( \Delta \)E. From our previous analysis, we know that cell [3;2] consistently performs poorly across cards. If we remove it, we get the following:
...
Our assumption was correct, it was the the cell [3;2]. Here we can make following observations:

1. \( \Delta \)E for skin colors is generally slightly lower, but has slightly higher variation than for non-skin colors. This is expected, as Douglas printing process is supposed to produce better skin colors.
2. Cell [3;2] is a consistent outlier across multiple cards.  This may indicate a problem either with a printer or dataset itself, depending on the reliability of the latter.
...
Is this result surprising. The 3rd order interaction model has 26 parameters to predict the 49 colors. Thus it is not surprising that it performs better predicting the data it was trained on compared to the simple linear model with only 6 parameters. More interesting is the comparison of the 2nd order interaction model and the 3rd degree polynomial regression since both have 16 parameters to estimate. Here the 2nd order interaction term performs much better with only 5 colors that show a color difference above 2 (compared to 39).

## Analysis of relationship with machine learning models

To check if the 3rd order interaction model tends to overfit we perform further tests. This time we split the 49 unique color fields of the master color card into a train and a test set. The training data keeps 33 rows and the test data consists of the remaining 16. We compare two different approaches:

a) We perform a bootstrap analysis. Therefor we take a fixed set of test data and perform 10000 regression with training data that is sampled with replacement from a common training data set. We calculate the median of the color difference for the 16 test colors every time. 
...
| Model | Method | Median | 5%-Quantile | 95%-Quantile |
|-------|--------|--------|-------------|--------------|
| Linear | Bootstrap | 5.4 | 3.7 | 8.1 |
| Linear | Full Resampling | 4.6 | 3.3 | 6.9 |
| 2rd order interaction | Bootstrap | 2.6 | 1.3 | 6.9 |
| 2rd order interaction | Full Resampling | 2.0 | 1.4 | 4.5 |
| 3rd order interaction | Bootstrap | 5.2 | 1.6 | 62.7 |
| 3rd order interaction | Full Resampling | 1.47 | 0.78 | 6.26 |
...
```{r, include=FALSE, cache=!full_refit}
results_i2_fullrepetition <- custom_boot(cbind(L, a, b) ~ .^3)
quantile(results_i2_fullrepetition, c(.05, .5, .95))
```