...
```{r colorcard, echo=FALSE, warning=FALSE, cache=!full_refit}
...
```{r colorcard first sheet, echo=FALSE, warning=FALSE, cache=!full_refit}
...
```{r CMYK and Lab master colorcard, echo=FALSE, warning=FALSE, cache=!full_refit}
...
```{r CMYK and Lab master colorcard difference, echo=FALSE, warning=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
**Why is there one brown color field without S?**

```{r S tile plot, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
```{r Lab correlations with S, echo=FALSE, warning=FALSE, message=FALSE, fig.width=10, fig.height=8, cache=!full_refit}
...
The L dimension appears to correlate strongly with S values, when excluding cases where S equals 0. **However, this correlation is misleading as our analysis indicates that L values do not significantly influence S values. (Which analysis?)**
...
**From my pattern detection feeling I'ld say that L and S might be correlated quadratically and that the trend for a,b and chroma should be pritty streight upwards (in the point dense clouds).**
...
```{r mean colorcard, echo=FALSE, warning=FALSE, cache=!full_refit}
display_color_card(mean_lab_color_card, "Row", "Col", "Lab", 2)
```

#### Analysis of field discrepancies

```{r colorcard mean vs master, echo=FALSE, warning=FALSE, fig.width=8, fig.height=10, cache=!full_refit}
...
```{r colorcard distributions, echo=FALSE, fig.height=6, warning=FALSE, fig.align='center', cache=!full_refit}
...
```{r dE_colorcard, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
```{r dE_colorcard_2, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
```{r dE_colorcard_3, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
color_differences %>% 
  filter(Sheet == 1, Row == 1, Column == 1) %>% 
  ggplot() +
...
```{r dE_colorcard_4, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
color_differences %>% 
  filter(Sheet == 1, Row == 1, Column == 1) %>% 
  ggplot() +
...
```{r Lab correlations with deltaE, echo=FALSE, warning=FALSE, message=FALSE, cache=!full_refit}
...
**maybe we should fix the y coordinate here or exclude the point 18**
...
```{r dE_colorcards_topleft_all_sheets, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
```{r dE_colorcards_1st_sheet, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
When we are looking at the \( \Delta \)E values for the color cards at target 1-1 and sheet 1 we find that the color difference tends to be noticeably higher for the color fields at target 1-1 in 20 cases and lower in 10 cases. Especially for the grey fields. The height of the boxes in contrast seems to be lower which is cautiously interpreted as a lower color variance. One has to keep in mind that the boxplots of target 1-1 consist of 13 data points and those of the sheet 1 of 42 data points.

```{r, echo=FALSE, echo=FALSE, fig.width=8, fig.height=12, cache=!full_refit}
...
```{r colors_only_CMYK, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
...
```{r, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit}
color_differences %>% 
