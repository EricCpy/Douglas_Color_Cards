...
scale_fill_gradien2_green_red <- scale_fill_gradient2(
    low = "#88CC00",
    mid = "white",
    high = "#CC3333",
    midpoint = 2,
...
The data given consists of two files. One file holds the master color information given in the Lab color space as well as in the CMYKS color space. Following we sow a summary of the master color data sets columns that hold the color values. Columns holding positional information for the color fields were excluded because they are distributed uniformly.
...
This average color card serves as a baseline against the reference data of the master. Below is a representation of the average color card.
...
Our examination reveals notable fluctuations across various fields. Particularly, the 18th field (3rd row, 2nd column) exhibits significant deviations from the master's data. Overall, it is evident that the mean of the color cards approximately matches the master in most other fields.
...
Comparing the first color cards colors with the master colors shows that there is one color that is pretty far from the master color (color field 18; 3rd row, 2nd column). There are some more fields where we spotted a difference at first glimpse:
...
To verify if the cutoff value of 2 holds for our example we show the same picture with highlighted colors that should be distinguishable in theory (dE > 2). According to this, we should be able to perceive differences in the following colors but we did not:
...
Finally we examine which color pairs should show no visible difference (dE < 1). Here we found no exception to the given rule of thumb.
...
### Color difference aggregated over multiple cards
...
In order to get a feeling if the color difference compared to the masters colors varies more or less regarding to the color cards position on the sheet or among the sheets at the same position one can have a look at small multiples for both cases (see Figures \@ref(fig:dEColorcards1stSheet), \@ref(fig:dEColorcardsTopLeftAllSheets)). Displaying all color cards of all sheets is possible on a huge screen but not in this report. Anyhow, as one can see, comparing the separated \( \Delta \)E tile maps visually is not easy.

But as a first step towards this comparison, we have a look at the mean values of \( \Delta \)E of specific card position across all sheets. We will find 42 values that don't correspond to the 64 color fields but to the color cards positions. Thus they are aggregate one level higher. 

First, we check numerically if there is any meaningful difference in mean \( \Delta \)E between different card positions on sheet. For that, we perform an ANOVA test. We assume that the color difference is higher for higher column values because ink might get low when the printer head goes from left to right. Thus we numerated the color fields positions by \(N_{card} = (N_{Column}-1) \cdot 7 + N_{Row}\) and are not using cardnumber as a factor.
...
As we can see, there is a significant evidence, that the mean \( \Delta \)E varies across different card positions (targets). The following graphic visualizes this. The color scale used differs from the color scale before to make clear that the values range is very different.

```{r, echo=FALSE, message=FALSE, cache=!full_refit, fig.cap="Mean dE across different card positions", fig.align='center', fig.width=8, fig.height=8}
diff_by_card %>% 
  group_by(Row, Column) %>%
  ...
  scale_fill_gradientn(colours = rainbow(2)[2:1])
```

We can see, that \( \Delta \)E generally goes up the further down we go and that there are three columns of higher \( \Delta \)E values. But those columns are not the high numbered but every second. Thus our hypothesis was not correct. Nevertheless a significant difference was found.

The exact meaning of the obtained information depends on the printing process involved. Even though the difference in the means is pretty low one has to keep in mind that this is already the mean of color difference. Thus we feel that a small difference in the mean could be a sign for a meaningful change.

Looking at all the individual color fields of all 42 color cards aggregated over the sheet shows that there might be two slight patterns. One might find *a)* a light diagonal from bottom left to top right (see cards in first row) or *b)* a dark diagonal from top left to bottom right (see cards in last row). We will investigate this further with other visualization techniques.
...
In contrast, aggregating the color cards over the 13 sheets (regardless their position on the sheet) does not show distinguishable patterns. They all look alike.
...
When we are looking at the \( \Delta \)E values for the color cards at target [1;1] ([Row;Column]) compared to those on sheet 1 (these two sets have 1 color card in common) we find that the color difference tends to be noticeably higher for the color fields at target 1-1 in 20 cases and lower in 10 cases. Especially for the grey fields (Figure \@ref(fig:deTargetFirstSheet)). To remember the corresponding color for each field we added a circle in this color to the box plots.

The height of the boxes in contrast seems to be lower which is cautiously interpreted as a lower color variance. One has to keep in mind that the boxplots of target [1;1] consist of 13 data points and those of the sheet 1 of 42 data points.

It should be noticed that field 18 was excluded again. Its color difference is above 20 and does not fit on the common y scale without squeezing everything else too hard.
...
In Figure \@ref(fig:MADdeAllSheets) we show boxplots of the MAD value of color difference comparing the generalized color variance among sheets and targets. Thus we find 13 values for the sheet box plot and 42 values for the target box plot this time.

We count 25 noticeably higher MAD values for sheets compared with targets MAD values. We find only three color fields where the opposite is the case. Thus we conclude that the color variance among the color cards on a sheet is higher than within color cards at the same target. It seems consequent to generate QR codes to inform the app which color card is scanned. This way the app can compare the detected color of the printed square with a target specific reference color (instead of the master color) for that field.
...
color_dispersion_compare %>% left_join(
  master_colors %>% select(Field, Crow, Ccol, Color) %>% unique(),
  by = join_by(Field)
) %>% #filter(Field != 18) %>% 
  ggplot() +
...
We haven't found an optimal way to display both the median color difference and the MAD of color variance in a single plot. However, we believe these box plots are a straightforward and an established visualization method for good reason.
...
In the following three graphics we want to look at one example on how the average color cards colors differ from the master and from a sample card at the corresponding target position. The average color card is created by aggregating all colors from cards in one target position (here [1;1]) (see note at [Comparing color space dimension individually]).

We begin with the graphic where we compare the sample color card with the master color card once again. This time we are using a color scale with a log2 scaling and a neutral center for filling of the tiles. Thus the comparison with the third color comparison should be more meaningful.
...
## Further color difference related analysis
...
```{r, echo=TRUE, cache=!full_refit, class.source = 'fold-hide'}
...
```{r regression1, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Master multivariate multiple linear regression Lab (left circle) compared to first color card Lab (right circle)", fig.align='center'}
color_differences %>% 
...
```{r, echo=FALSE, cache=!full_refit, class.source = 'fold-hide'}
degree <- 3
...
```{r regression2, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Master multivariate multiple polynomial regression Lab (left circle) compared to first color card Lab (right circle)", fig.align='center'}
color_differences %>% 
  filter(Sheet == 1, Row == 1, Column == 1) %>% select(-Difference, -L, -a, -b) %>% bind_cols(
    color_differences_regression, predict(color_regression, master_colors)
  ) %>% rowwise() %>% mutate(Color = lab_to_rgb(L, a, b)) %>% 
...
```{r regression3, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Master multivariate multiple linear regression with interactions Lab (left circle) compared to first color card Lab (right circle)", fig.align='center'}
color_differences %>% 
  filter(Sheet == 1, Row == 1, Column == 1) %>% select(-Difference, -L, -a, -b) %>% bind_cols(
    color_differences_regression, predict(color_regression, master_colors)
  ) %>% rowwise() %>% mutate(Color = lab_to_rgb(L, a, b)) %>% 
...
a) We perform an ordinary bootstrap analysis. Therefore we take a fixed test data set and perform 10000 regressions with a fixed training data set that is sampled from with replacement. We calculate the median of the color difference for the 16 test colors every time. 
b) We perform 10000 different train test splits and do the regressions once each (calling it full resampling). Therefore we use permutative bootstrapping. We still calculate the median of the color difference between predicted and correct test colors.

| Model | Method | t | Bias | 5%-Quantile | Median | 95%-Quantile |
|-------|--------|--------|-------------|--------------|--|--|
| Linear | Ordinary Bootstrap                        | 5.2 | 0.27 | 3.93 | 5.35 | 7.63 |
| Linear | Full Resampling                  | 4.1 | 0.63 | 3.26 | 4.54 | 6.85 |
| 2rd order interaction | Ordinary Bootstrap         | 1.4 | 1.63 | 1.45 | 2.62 | 5.11 |
| 2rd order interaction | Full Resampling   | 2.9 | -0.43 | 1.43 | 2.06 | 4.43 |
| 3rd order interaction | Ordinary Bootstrap         | 2.1 | 8.22 | 1.87 | 5.11 | 37.9 |
| 3rd order interaction | Full Resampling   | 2.2 | -0.01 | 0.78 | 1.44 | 6.20 |

Comparing the results stated in the upper table for the ordinary bootstrapped t values (or alternatively medians) with those of the full permutative resampling t values yields no clear pattern. One is higher, one lower, one almoast equal. The confidence intervals for the ordinary bootstrapped values tend to be wider but not always is the narrower CI fully included in the wider. The linear model has highest t value for color difference medians and high CI limits. (A polynomial regression coudln't be fitted because of to few data points.)

The t values for 2nd and 3rd order interaction model are lower or almost equal with ordinary bootstrapping. In contrast one finds the CI limits to be lower for bootstrapping with permutation. The CIs for 2nd order interaction models are narrower and the upper CI limit is lower compared to 3rd order models.

Thus one might conclude that 2nd order models are less likely to overfit. For the 3rd order interaction regression one can be pretty lucky and get a really good model but one can be very unlucky as well and end up with a median \(\Delta\)E of 37.9 which yields colors that are pretty different. At this stage of analysis we would suggest to go with the 2nd order interaction regression model instead of a 3rd order interaction model. But further analysis was carried out and we came to a different conclusion (see below).

One can visualize this with classical box plots or alternatively with the fancier violin plots (Figure \@ref(fig:violinAndBoxplots)). One sees that there are very extreme values found for the interaction models that seem to be normal-ish distributed all the way up above 125.
...
results_all_bootstrap <- boot(data=train_data, statistic=color_difference,
                                   R=10000, formula=list(cbind(L, a, b) ~ C + M + Y + K + S, cbind(L, a, b) ~ .^2, cbind(L, a, b) ~ .^3))
...
results_all_bootstrap_differences_perc <- results_all_bootstrap_differences %>% summarise_all(~mean(.>0))
```
...
results_all_bootstrap_perm_differences <- results_all_bootstrap_perm$t %>% as_tibble() %>% 
  mutate("lm vs i2" = V1-V2, "lm vs i3" = V1-V3, "i2 vs i3" = V2-V3) %>%  select(-V1, -V2, -V3)
results_all_bootstrap_perm_differences_perc <- results_all_bootstrap_perm_differences %>% summarise_all(~mean(.>0))
```

```{r violinAndBoxplots, echo=FALSE, fig.width=8, fig.height=12, cache=!full_refit, warning=FALSE, fig.cap="Comparing median dE values from bootstrap analysis"}
bootstrap_results <- results_all_bootstrap_perm$t %>% as_tibble() %>% 
  ...
  mutate(formula = factor(formula, levels = c("linear", "interaction 2nd order", "interaction 3nd order")))

v1 <- bootstrap_results %>% ggplot() +
  geom_violin(aes(x = formula, y = value, fill = method), draw_quantiles = c(0.5)) +
  xlab("model type") + ylab("median dE") + labs(fill = "bootstrap method")

v2 <- bootstrap_results %>% ggplot(aes(x = formula, y = value, fill = method)) +
  geom_violin(draw_quantiles = c(0.25, 0.5, 0.75)) +
  coord_cartesian(ylim = c(0, 10)) +
  xlab("model type") + ylab("median dE") + labs(fill = "bootstrap method")

b1 <- bootstrap_results %>% ggplot() +
  geom_boxplot(aes(x = formula, y = value, fill = method)) +
  xlab("model type") + ylab("median dE") + labs(fill = "bootstrap method")
...
In this case we prefer the box plots for presentation over the violin plots. Because the points are distributed unimodal the box plot representation is well suited. But for analysis if the density is really normal-ish the violin plots are helpful.

A visual disadvantage of the box plots seem to be the stacked outlier points. With less bootstrap results this would be a lesser problem. Here the black line of the violin plot shows that there are some extreme values on a nicer way. But this also has its own disadvantage: the long streched violin plot gets very slim and thus it becomes hard to spot the quartile lines.

A visual advantage of the box plots is that they show faster if a T test would be significant or not. We'ld like to show this with the violin plots as well and had the idea to remove the filling outside of the quartiles. Unfortunatly ggplot has no built in option for this and solutions found on stack overflow work only for equally long violin plots. 

The core problem why the workarounds don't yield nice graphics is that one can't adjust the number of points where the density is evaluated (for stat_ydensity and geom_violin) ([compare last comment in this Stack Overflow](https://stackoverflow.com/questions/76088376/how-to-plot-difficult-probability-distributions-with-ggplot2)). This is only available for the regular density.

We found multiple approaches:

1. Drawing colored lines on each evaluated density row with geom_segment and ggplot_build.[1](https://stackoverflow.com/questions/41087157/gradient-fill-violin-plots-using-ggplot2), [2](https://stackoverflow.com/questions/67077580/how-to-make-a-violin-plot-gradient-fill-as-a-colour-scale-based-on-y-axis)
1. Advanced to drawing colored rects on each evaluated density row with geom_rect and ggplot_build using lag and lead.
1. Drawing a polygon based on the evaluated density rows with geom_poly and ggplot_build. [1](https://stackoverflow.com/questions/34261038/ggplot2-violin-plot-fill-central-95-only), [2](https://stackoverflow.com/questions/36203195/fill-specific-regions-in-geom-violin-plot)
1. Advanced to drwaing a polygon based on the evaluated density rows with geom_poly and ggplot_build adding density approximation on quartiles.

```{r, echo=FALSE}
mywidth <- .225 # bit of trial and error

df_input <- bootstrap_results %>% mutate(group = str_c(formula, method))
quantiles <- df_input %>% group_by(as.character(formula), method) %>% summarise(q25 = quantile(value, .25), q75 = quantile(value, .75), .groups = "drop")

p <- ggplot(df_input, aes(x=as.integer(formula),y=value, fill = method, group = group)) + stat_ydensity() + scale_y_continuous(limits = c(0, 10), 
                                                                                                                   oob = scales::oob_keep)
...
First we found an approch that draws lines on each evaluated density point. For high violins this results in stripes.

```{r violinsLineGradient, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Violin plots with geom_segment", fig.align='center'}
ggplot() +
  ...
  geom_violin(
    data = df_input, 
    aes(x = as.integer(formula), y = value, fill = method, group = group), 
...
A first approach to fill the gaps was to use rectangles instead of lines that get drawed to the next data rows y values. This gets pretty edgy and can lead to colors out of the violin.

```{r violinsRectGradient, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Violin plots with geom_rect", fig.align='center', warning=FALSE}
ggplot() +
  ...
  geom_violin(
    data = df_input, aes(
      x = as.integer(formula), y = value, fill = method, group = group), 
    draw_quantiles = c(0.25, 0.5, 0.75), alpha = 0, show.legend = FALSE) + 
  labs(x = "model", y = "median dE") +
  coord_cartesian(ylim=c(0,10)) +
  scale_x_continuous(breaks = 1:3, labels = c("linear", "interaction 2nd order", "interaction 3nd order"))
...
Another approach was to get polygons out of the ggplot_build data. Because of the low number of evaluation points this lead to gaps at the bottom or upper end.

```{r violinPolygonGradient, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Violin plots with geom_polygon", fig.align='center'}
dat <- bootstrap_results %>% mutate(group = str_c(formula, method)) %>% dplyr::rename(y = "value")
quantiles <- dat %>% group_by(formula, method) %>% summarise(q25 = quantile(y, .25), q75 = quantile(y, .75), .groups = "drop")
...
ggplot() + 
  ...
  geom_polygon(data = p_build %>% filter(fill_group == "ja"),
               aes(x = x,y = y,group = group, fill = fill), show.legend = F)+
  ggnewscale::new_scale_fill() +
...
Our final approach was to get an additional estimate at the upper and lower quartile. We still find some gaps at the lower quartile and at the upper quartile the color gets drawn a little bit over the quartile line. This is because the drawn quartile lines are based on the density estimate and the filled area is based on the quartiles calculated from the raw data (simply counted out). This is a well known issue we won't tackle in the last two hours before the report is to be handed in.

```{r violinPolygonGradientQuartiles, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Violin plots with geom_polygon with approximated quartile density", fig.align='center'}
p <- ggplot() + geom_violin(data = dat,aes(x = factor(formula),y = y, fill = method))
p_build <- ggplot2::ggplot_build(p)$data[[1]] 
...
But we find that coloring only the quartile area of the violin is a promising step for better interpretable alternative to classical box plots. Even though the problem with to slim violins was not solved this way.

```{r violinPolygonGradientQuartilesVSboxplots, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, fig.cap="Violin plots with geom_polygon with approximated quartile density", fig.align='center'}
ggpubr::ggarrange(
  violin_final, b2,
  ncol = 2, nrow = 1,
  common.legend = TRUE,
...
### Fight the uncertainty with bootstrapping

But how high is the chance that one gets a worse result with a 3rd order interaction model than with a competing model? And how high is the chance that one gets a better result with a 2rd order interaction model than with any other competing model?

Since we did the regression for all three models with the same train test split in each bootstrap step we can directly compare the color difference median values from the bootstrap t data set. If the difference for two models medians is positive the better model is the one thats median was subtracted from the other one.

(If the models were evaluated in independent bootstrap evaluations one will find that the train test data split was different for two models that lead to the result in the same row of the bootstrap data. One can still get a approximation for the statistic by randomly selecting two models and compare their color difference repeatedly with replacement.)

We found from the ordinary bootstrap data:

* In `r results_all_bootstrap_differences_perc[1]*100` % of the cases the 2nd order interaction model yields a better result than the linear model.
* In `r 100-results_all_bootstrap_differences_perc[3]*100` % of the cases the 2nd order interaction model yields a better result than the 3rd order interaction model.
* In `r results_all_bootstrap_differences_perc[2]*100` % of the cases the 3nd order interaction model yields a better result than the linear model.

This suggests that one should pick the 2nd order interaction model if one wants to get better results compared to the simple linear model with a high chance. The 2nd order interaction model seems to bet better than the 3rd order interaction model in majority of the cases as well.

In the following graphic one can see additionally that the densities for the color difference median has really thick tails to the negative side if the 3rd order interaction model is part of the comparison. This shows that the difference between the performance of the models can be really huge. One might not only get a over parameterized model that gets a little bit worse suggestions. It is possible to get a model that predicts really weird colors.

```{r, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, warning=FALSE, message=FALSE, fig.cap="Comparing median dE difference values for competing models from ordinary bootstrap analysis"}
results_all_bootstrap_differences %>% 
  ...
  geom_text(
    data=tribble(
      ~x, ~y, ~label, ~compare,
      -2.5, 0.2, str_c(100-results_all_bootstrap_differences_perc[[1]]*100, " %"), "lm vs i2",
      2.5, 0.2, str_c(results_all_bootstrap_differences_perc[[1]]*100, " %"), "lm vs i2",
      -2.5, 0.2, str_c(100-results_all_bootstrap_differences_perc[[2]]*100, " %"), "lm vs i3",
      2.5, 0.2, str_c(results_all_bootstrap_differences_perc[[2]]*100, " %"), "lm vs i3",
      -2.5, 0.2, str_c(100-results_all_bootstrap_differences_perc[[3]]*100, " %"), "i2 vs i3",
      2.5, 0.2, str_c(results_all_bootstrap_differences_perc[[3]]*100, " %"), "i2 vs i3",
    ), aes(
      x = x, y = compare, label = label
    ), vjust = 1.5, hjust=.5
    ) +
  ylab("models compated") + xlab("median dE") +
  labs(fill = "complexer model performs better")
```
...
* In `r results_all_bootstrap_perm_differences_perc[1]*100` % of the cases the 2nd order interaction model yields a better result than the linear model.
* In only `r 100-results_all_bootstrap_perm_differences_perc[3]*100` % of the cases the 2nd order interaction model yields a better result than the 3rd order interaction model.
* In `r results_all_bootstrap_perm_differences_perc[2]*100` % of the cases the 3nd order interaction model yields a better result than the linear model.

Choosing a 2nd or 3rd order interaction model yields better results compared to the simple linear model with a comparable in most of the cases. Both options seem to be pretty safe. But in this analysis one gets better results with the 3rd order interaction model compared to the 2nd order interaction model with a chance of `r results_all_bootstrap_perm_differences_perc[3]*100` %. From this results we would recommend to go with a 3rd order interaction model. Since there are no fat tails in the corresponding density plot the thread to get a weird 3rd order interaction model seems lower than the possible benefit of a model that performs significantly better.

```{r, echo=FALSE, fig.width=8, fig.height=8, cache=!full_refit, warning=FALSE, message=FALSE, fig.cap="Comparing median dE difference values for competing models from permutative bootstrap analysis"}
results_all_bootstrap_perm_differences %>% 
  ...
  geom_text(
    data=tribble(
      ...
      -2.5, 0.2, str_c(round(100-results_all_bootstrap_perm_differences_perc[[2]]*100, 2), " %"), "lm vs i3",
...
One possible explanation for these very different results could be that the chose train test split for the ordinary bootstrapping was unlucky and non representative. It is also possible that resampling with replacement is a severe problem in this case because we have only few training data and it could happen very fast that all colors of one chroma segment of the color wheel are missing completely. Repeating the ordinary bootstrapping approach with four different seeds lead to one better, one similar and two worse result for the 3rd order interaction model. But we are not totally sure if there is a methodical flaw with the full resampling approach either.

In general for a case like this we would suggest to get more data and repeat the analysis again. Or to perform a sensitivity analysis with simulated data. If both is not possible one should inspect the predictions from a 3rd order interaction model before deployment. If this is not possible as well (e. g. because one redeploys often) it depends on how sensible the case of usage for this model might be.

One should take the safer option (2nd order interaction model) if the colors should be used in a traffic light and wrong colors could lead to traffic accidents, injuries and death. If its just the color of an add in a video game it does not matter. But even here one would be totally fine with the 2nd order interaction model and it would be computative less costly as well. I challenge the reader to come up with a use case where the small advantage of a good 3rd order interaction model could be crucial.

# Conclusion

Based on our analysis, we have identified significant variations in color reproduction across different fields. Especially field 18 consistently has notable perceptible differences from the master colors. This may be the result from printer errors or inaccuracies in the printing data. Additionally, colors such as lilac, blue, and darker shades have easily perceptible differences. Colors like magenta, olive, and lavender also show differences that should be perceivable but are not detectable on our devices.
Furthermore, our report reveals that luminance strongly influences perceived color differences in the printed cards. This suggests a potential challenge in accurately reproducing luminance values.

We also observed that color variance among color cards on a sheet is higher than within color cards at the same target. To address this Douglas generates QR codes to inform the app which color card is being scanned. This approach seems useful because it enables the app to compare the detected color of the printed field with a target specific reference color for that field, rather than the master color.

Moreover, our report emphasizes the importance of the S value in CMYKS for determining skin tones and converting CMYKS to the Lab color space. By leveraging advanced machine learning models such as multivariate multiple regression and polynomial regression, we have developed robust methods for predicting Lab values from CMYKS inputs. We would recommend employing a 2nd order interaction model, which effectively balances complexity and performance to deliver reliable results for our color samples.

## Learnings and further notes

* During our group work we all found some new details in the others R code. For example: I used the *ungroup* command after *summarise* but I learn from Pavlo that there is a *.groups = "drop"* parameter within the *summarise* command for this task. 
* We found out how to display uncertainty areas with a ribbon instead of two CI lines. 
* We used *grid.arrange* and facets a lot to create small multiples that really help comparing multiple related plots.
* We discussed about how to best specify out hypotheses as a model. For example regarding the question if we should use card number as a factor or as numeric.
* We have concentrated to establish common color scales for similar visualization and using other ones for differing scales. It seems really hard to have a set of distinguishable colors when each color should be linked to something all over the report.
* It can also be helpful to scale the color scale not only the x or y axis to show differences over large value ranges.
* Showing the development of our thoughts incorporates a lot of similar figures. In a report for a CEO we probably pick just one from each series. But to foster understanding a stepwise evolution seems to be pretty helpful.
* When talking about colors it is handy to present the color that is talked about along with other statistics (like box plots).
* New and fancy visualizations are not always better. We found violin plots to have some problems and tackeled one of those.
* The heavy package is archived. To perform a outlier resistant regression (with T distributed errors) one could user bayesian regression with *brms*. But we didn't perform this analysis here. The trend spotted with ones eye can get disturbed really quick by some off values.
* We are not sure if our ordinary bootstrap approach is faulty or if the 3rd order interaction model does really overfit very strongly and the permutative bootstrapping approach is wrong. We should contact an expert about this instead of hoping that the answer we like more is true or being too defensive with our decision.
* We learned that it is easy to forget how to do some specific things with R / ggplot. Even though I worked on a visualization package for bayesian models I forgot how to color geom_ridges differently. But once you know that it is possible one finds possible solutions on Stack Overflow fast. Maybe I should have tested an generative AI on this (or the violins) question? I think when I had to redo this I'd ask a generative AI to help me with some coding tasks.
