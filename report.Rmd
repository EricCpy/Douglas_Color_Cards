...
color_differences %>% 
  ...
  geom_point(aes(x = Ccol+0.25, y = Crow, color = Color), size = 11) +
  scale_color_identity() +
  geom_point(
    data = master_colors %>% rowwise() %>% mutate(Color = lab_to_rgb(L, a, b)),
    aes(x = Ccol-0.25, y = Crow, color = Color), size = 11
    ) +
  coord_fixed() +
...
color_differences %>% 
  ...
  ggplot() +
  geom_tile(aes(fill = Difference, x = Ccol, y = Crow)) +
  geom_point(aes(x = Ccol+0.2, y = Crow, color = Color), size = 12) +
  scale_color_identity() +
  geom_point(
    data = master_colors %>% rowwise() %>% mutate(Color = lab_to_rgb(L, a, b)),
    aes(x = Ccol-0.2, y = Crow, color = Color), size = 12
    ) +
  coord_fixed() +
  labs(caption = "Left circle: master color; Right circle: sample color") +
  xlab("Column") + ylab("Row") +
