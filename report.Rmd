...
To compare the color spaces, we generated a mean color card derived from derived from all color cards in our *Lab_Measurement* dataset. 
...
To adjust the height of the plots in your R Markdown document, you can use the `fig.height` chunk option within your code chunk that generates the plots. Here’s how you can modify your code chunk:
...
```{r Lab correlations with deltaE, echo=FALSE, warning=FALSE}
...
With a multivariate multiple linear regression we find `r sum(color_differences_regression > 2)` colors that should be different on first glimpse.
...
With a multivariate multiple polynomial regression with degree `r degree`  we find `r sum(color_differences_regression > 2)` colors that should be different on first glimpse. (38 colors with 2nd degree polynomials.) This are almost as many as with the linear regression. We even find some colors that have been matched better with linear regression.
...
With a multivariate multiple linear regression including interaction terms up to 3rd order we find `r sum(color_differences_regression > 2)` colors that should be different on first glimpse. (Five colors with only 2nd order interaction terms.)
