...
```{r Lab correlations with S, echo=FALSE, warning=FALSE, message=FALSE, fig.width=8, fig.height=8}
grid.arrange(plot_correlation(master_colors, "L", "S"),
...
Notably, the S values show strong correlations with 'a' and 'b', while showing a weaker correlation with Chroma. Chroma in the Lab color space quantifies the saturation or colorfulness of a color. It measures how far a color is from being neutral (gray). Chroma in the Lab color space is calculated using the formula:

\[ C = \sqrt{a^2 + b^2} \]

where 'a' and 'b' represent the color opponent dimensions in the Lab color model. Notably, Pearson correlations are lower than Spearman rank correlations, suggesting that the relationship between S and Lab values is likely not strictly linear.
This observation prompts further investigation into how S values interact with Lab color space.
...
Examining the distribution of values across individual channels provides insights into the predominant brightness and color characteristics.
**To adjust the height of the plots in your R Markdown document, you can use the `fig.height` chunk option within your code chunk that generates the plots. Here’s how you can modify your code chunk: (delete this?)**

```{r colorcard distributions, echo=FALSE, fig.height=6, warning=FALSE, fig.align='center'}
plots <- plot_density_vs_master(mean_lab_color_card, master_colors)
grid.arrange(plots[["L"]], plots[["a"]], plots[["b"]], nrow = 3)
```

**Lower and upper density curves switch position relative to card. Can we make ribbons with alpha instead of two lines?: See [here](https://stackoverflow.com/questions/45908120/add-shaded-standard-error-curves-to-geom-density-in-ggplot2).**

The distributions reveal distinct characteristics across the channels: 

- The **L** dimension follows a normal distribution, indicating a tendency around a specific brightness level.
- Both **a** and **b** dimensions have bimodal distributions, suggesting a limited spread of colors in our dataset.

**don't agree with the above (Simon)**

Further analysis highlights specific discrepancies:
...
- **b dimension:** Similar to 'a', deviations are observed especially around 'b' values between -10 and 40, indicating representations that differ from the master in both higher and lower ranges.
...
**Should we set scale_fill_continuous(limits = c(0, 10)) for more comparable scales?**
